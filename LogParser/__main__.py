...
print("                Sources:")
for source in parser._sources_60min:
    print(f"   {source:>20}: {parser._sources_60min[source]:7,.0f}")
print("                Targets:")
for target in parser._targets_60min:
    print(f"   {target:>20}: {parser._targets_60min[target]:7,.0f}")

print( "\n\n Damage by type in last 60 minutes:")
print("                Sources:")
for type_to in parser._type_to_60min:
    print(f"   {type_to:>20}: {parser._type_to_60min[type_to]:7,.0f}")
print("                Targets:")
for type_from in parser._type_from_60min:
    print(f"   {type_from:>20}: {parser._type_from_60min[type_from]:7,.0f}")

print( "\n\n Damage by system in last 60 minutes:")
print("                Sources:")
for system_to in parser._system_to_60min:
    print(f"   {system_to:>30}: {parser._system_to_60min[system_to]:7,.0f}")
print("                Targets:")
for system_from in parser._system_from_60min:
    print(f"   {system_from:>30}: {parser._system_from_60min[system_from]:7,.0f}")